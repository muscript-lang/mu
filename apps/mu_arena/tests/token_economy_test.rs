...
#[test]
fn mu_arena_compressed_token_economy_targets() {
    ...
    for path in ["apps/mu_arena/src/main.mu", "apps/mu_arena/src/runner.mu"] {
        let src = fs::read_to_string(path).expect("mu_arena source should exist");
        let program = parse_str(&src).expect("mu_arena source should parse");
        ...
        println!(
            "mu_arena token economy [{path}]: readable_bytes={readable_bytes} compressed_bytes={compressed_bytes} readable_tokens={readable_tokens} compressed_tokens={compressed_tokens} symtab_size={symtab_size} avg_symref_width={file_avg_symref_width:.2} max_symref_width={file_max_symref_width}"
        );
    }
    ...
    println!(
        "mu_arena token economy [total]: readable_bytes={total_readable_bytes} compressed_bytes={total_compressed_bytes} readable_tokens={total_readable_tokens} compressed_tokens={total_compressed_tokens} symtab_size={total_symtab_size} avg_symref_width={avg_symref_width:.2} max_symref_width={max_symref_width}"
    );
